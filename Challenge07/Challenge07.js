...
const objLat = (obj) => {
    ...
    // let fLitter = obj.firstName[0].toUpperCase();
    // let lLitter = obj.lastName[0].toUpperCase();
    let fN = obj.firstName + "";
    fN = fN.replace(fN.charAt(0), fN.charAt(0).toUpperCase());
    let lN = obj.lastName + "";
    lN = lN.replace(lN.charAt(0), lN.charAt(0).toUpperCase());

    let introMsg = `my name is ${fN} ${lN} I am ${obj.age} YO, and I love ${obj.hobby}.`
    return introMsg;
...
const cvFormatter = (arr) => {
    ...
    for (let x = 0; x < arr.length; x++) {
        if (arr[x].firstName === null || arr[x].LastName === null) {
            ...
            fName[x] = arr[x].firstName + " " + arr[x].LastName;
    ...
    for (let i = 0; i < cv.length; i++) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            if (arr[i].yearsOfExperience > 1)
                cv.push({ fullName: fullName[i], tech: arr[i].tech });
        }
    }
    console.log(cv);
...
const applicationsStatics = (cvs) => {
    let cv = cvFormatter(cvs);
    let result = {
        python_Devs: 0,
        javaScript_Devs: 0,
        dotNet_Devs: 0,
    ...
    result.totalApplicants = cvs.length;
    console.log(CV.length);
    result.rejectedApplicants = cvs.length - CV.length;
    console.log(result.rejectedApplicants);
    for (let x = 0; x < cvs.length; x++) {
